...
- (void)sd_internalSetImageWithURL:(nullable NSURL *)url
    ...
    if (!(options & SDWebImageDelayPlaceholder)) {
        dispatch_main_async_safe(^{ //采用 placeholder 来使用展位图临时填充当前 UIImageView
    ...
    if (url) {
...
#if SD_UIKIT || SD_MAC
        // check and start image indicator 指标
        // 开启瞎子啊进度显示
        ...
        id <SDWebImageOperation> operation = [manager loadImageWithURL:url options:options context:context progress:combinedProgressBlock completed:^(UIImage *image, NSData *data, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
            ...
            SDWebImageNoParamsBlock callCompletedBlockClojure = ^{
                ...
                //当前 image 不为空对下载后 Image 做操作 | 获取图片为空并且不是延迟加载  为非
                //在返回获取的 Image 先更新 layoutSubView
            ...
            if (image) { ///当前在 image 的图片 --> 设置目标的 Image 图片
...
#if SD_UIKIT || SD_MAC
                ///
                //给当前 UIImageView | UIButton 设置当前背景图片 
        ...
        //把当前下载操作保存在当前 操作 MapTable 中
        [self sd_setImageLoadOperation:operation forKey:validOperationKey];
    } else {
#if SD_UIKIT || SD_MAC
        ///在下载 URL 为 nil 时，停止当前下载展示 & 返回错误结果
        [self sd_stopImageIndicator];
#endif
        dispatch_main_async_safe(^{
            if (completedBlock) {
...
- (void)sd_setImage:(UIImage *)image imageData:(NSData *)imageData basedOnClassOrViaCustomSetImageBlock:(SDSetImageBlock)setImageBlock transition:(SDWebImageTransition *)transition cacheType:(SDImageCacheType)cacheType imageURL:(NSURL *)imageURL {
    ...
    } else if ([view isKindOfClass:[UIImageView class]]) { //当前是 imageView 来设置当前 imageView 的 iamge
...
#if SD_UIKIT
    else if ([view isKindOfClass:[UIButton class]]) { //在 iOS 判断当前 UIButton 来设置当前的图片
...
#endif
#if SD_MAC
    else if ([view isKindOfClass:[NSButton class]]) { //在 Mac 设置 NSButton
